...
class MetricsService {
  ...
  evaluateStability(m) {
    ...
    if (m.errorRate > this.THRESHOLDS.errorRate.critical) {
      reasons.push(
        `Error rate too high (${m.errorRate} > ${this.THRESHOLDS.errorRate.critical})`
      );
    }
    ...
    if (m.trafficRecovery < this.THRESHOLDS.trafficRecovery.critical) {
      reasons.push(
        `Traffic recovery too low (${(m.trafficRecovery * 100).toFixed(0)}% < ${(this.THRESHOLDS.trafficRecovery.critical * 100).toFixed(0)}%)`
      );
    }
    ...
    if (m.p95LatencyBefore > 0 && m.p95LatencyAfter > 0) {
      ...
      if (changePercent > this.THRESHOLDS.latencyRegression.critical_change_percent) {
        reasons.push(
          `Latency regression too large (${changePercent.toFixed(1)}% > ${this.THRESHOLDS.latencyRegression.critical_change_percent}%)`
        );
      }
    }
  ...
  calculateResilienceScore({ p95LatencyAfter, errorRate, trafficRecovery = 1 }) {
    ...
    const score = Number(
      (0.4 * latencyScore + 0.3 * errorScore + 0.3 * trafficScore).toFixed(3)
    );
